...
class ChartController {
    async dataForDashboard(req, res) {
        try {
            ...
            const countCompleteTicketToDay = await Ticket.find({status: 'Hoàn thành', updatedAt: {$gte: startOfDay(toDay), $lt: endOfDay(toDay)}}).count()
            const countAllCompleTicket = await Ticket.find({status: 'Hoàn thành'}).count()
